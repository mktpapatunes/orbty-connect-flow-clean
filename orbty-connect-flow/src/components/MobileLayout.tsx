...
import { LayoutDashboard, Plus, Clock, User, Home, LogOut } from "lucide-react";
...
const contractorNav = [
  ...
  // ✅ histórico removido do contractor
    ...
    <div className="mobile-container flex flex-col bg-background">
      ...
      <header className="sticky top-0 z-30 bg-background/80 backdrop-blur-xl border-b border-border/50 px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          <h2 className="font-display font-semibold text-foreground text-lg flex-1">{title}</h2>
          {showHome && (
            <button
              onClick={handleHome}
      ...
      {showNav && (
        <nav className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md z-30 bg-card/90 backdrop-blur-xl border-t border-border/50">
          {isContractor ? (
            // ✅ NAV CONTRACTOR: Início | (Criar destacado) | Perfil
            <div className="relative flex items-center justify-between py-2 px-6">
              ...
              <button
                ...
                className={`flex flex-col items-center gap-1 py-2 px-3 rounded-lg transition-all duration-200 ${
                  location.pathname === "/dashboard-contratante" ? "text-primary" : "text-muted-foreground"
                }`}
                aria-label="Início"
              >
              ...
              <div className="absolute left-1/2 -translate-x-1/2 -top-6">
                <button
                  ...
                  className="relative w-14 h-14 rounded-full bg-gradient-neon glow-blue flex items-center justify-center shadow-lg transition active:scale-[0.98]"
                  aria-label="Criar campanha"
                  title="Criar campanha"
                >
                  {/* anel suave */}
                  <span className="pointer-events-none absolute inset-0 rounded-full ring-4 ring-background/60" />
            ...
            // ✅ NAV INFLUENCER: mantém como estava
